...
class Food(Sprite):
    ...
    def is_near(self, point: Tuple[float, float], radius: float) -> bool:
        """Check if food is within radius of point."""
    ...
    def hit(self, spr: Sprite) -> bool:
        """
        Check if touching sprite.

        Example:
            f = Food(0, 0, 1)
            p = Player(0, 10, 10, 10, "red")
            f.hit(p) -> True (distance < combined size)
        """
...
class FoodList:
    ...
    def __sub__(self, other: "FoodList") -> "FoodList":
        """Remove items in other from self."""
    ...
    def reduce(self, func: Callable[[Food, Food], Food], initial: Optional[Food] = None) -> Optional[Food]:
        """Reduce food list to single value."""
        if not self.food:
            return initial
    ...
    def populate(self, amount: int, bounds: Tuple[int, int]) -> List[Food]:
        """
        Add random food items within bounds.

        Example:
            fl = FoodList([])
            fl.populate(5, (500, 500)) -> 5 foods in 500x500 area
        """
    ...
    def eat(self, chr: Character) -> List[Food]:
        ...
        Example:
            fl = FoodList([Food(0, 0, 10)])
            p = Player(0, 0, 10, 10, "red")
            fl.eat(p) -> removes food, increases player count
        """
        for f in self.food:
            if f.hit(chr):
                chr.eat()
    ...
    def move(self, bounds: Tuple[int, int] = (1280, 720)) -> Self:
        """
        Randomly drift all food slightly.

        Example:
            fl.move() -> all food moves by -1 to 1 in x and y
        """
        for f in self.food:
